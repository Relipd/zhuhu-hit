...
#   · 保留时间维度, 但**不用时间做区隔** —— date 只是条目属性(首次收录日期), 一条 url 只一行,
#     不按日期分桶、不因多日出现而重复入库;
#   · **聚合维度到 type**(案例/人物/链路) —— 只有 3 个稳定取值, 适合当分组维度;
#     cat 是自由文本主题标签, 实测 19 个分类里已出现近义重复(消费电子 vs 半导体与消费电子),
#     只作标签, 不参与聚合。
LIB_FIELDS = ("date", "type", "cat", "content", "url")
